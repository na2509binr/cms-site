...
export default function InsertUpdateMemberForm({ initialData, fetchMembers }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (activeTab === "insert") {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/member/insert`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formData),
      });
    } else {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/member/update`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formData),
      });
    ...
    setFormData({ name: "", image: "", department: "", order: 0, isActive: true, showWeb: true });
    setActiveTab("insert");
    fetchMembers();
  };
    ...
    <div className="max-w-md mx-auto p-6 bg-white rounded shadow">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div>
            <label className="block mb-1 font-medium">Bộ phận</label>
            <input name="department" value={formData.department ?? ""} onChange={handleChange} className="w-full border rounded px-3 py-2" />
          </div>
